...
module.exports = function fetchAdapter(config) {
  return new Promise(function dispatchFetchRequest(resolve, reject) {
    ...
    request.handleLoad = function(response) {
      if (!response.ok) {
        reject(createError('Internal Server Error', config, response.status, request, response));
        return;
      }
      response.json().then(function dispatchRequestJSON(data) {
        if (data.hasError) {
          reject(createError('JSON Date Error', config, response.status, request, response));
        } else {
          var rep = {
            data: data,
